...
def display_results(with_optional=False):
    ...
    with left_paper:
        ...
        if st.session_state.data:
            ...
            if  st.session_state.data_down:
                left_down, right_down = st.columns([3, 7])
                with left_down:
            ...
            utterance = st.session_state.data.get("utterance")
            if utterance:
    ...
    if right_paper is not None:            
        with right_paper:
            if st.session_state.data:
                if "prev_data" not in st.session_state or st.session_state.prev_data != st.session_state.data:
                    ...
                    st.session_state.pop("entity", None)
...
with st.sidebar:
    with st.container(border=True):
        ...
        if uploaded_file is not None:
            # Cập nhật session state với tệp mới nhưng không thay đổi audio_url ngay lập tức
...
with st.container():
    ...
    if submit_button or st.session_state.retry_button:
        if uploaded_file:
            if 'uploaded_file' in st.session_state and st.session_state.uploaded_file == uploaded_file.name:
                st.session_state.audio_url = save_url_audio(uploaded_file)
